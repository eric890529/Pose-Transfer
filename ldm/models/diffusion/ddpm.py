...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.old_model = None
        # self.old_first_stage_model = None
        # self.old_cond_stage_model = None
        # self.old_control_model = None
        # self.old_style_encoder = None
    ...
    def get_input(self, batch, k, is_inference=False):
        x = batch[k]
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx ):#dataloader_idx
        ...
        # old_cond_stage_model = copy.deepcopy(self.cond_stage_model)
        # global old_control_model 
        # old_control_model = copy.deepcopy(self.control_model)
        # global old_style_encoder 
        # old_style_encoder = copy.deepcopy(self.style_encoder)
    ...
    #             break
        ...
        #     if not torch.equal(tempList[i], tempList2[i]):
        #         print(model_name + " change")
        #         break
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, return_x=False, is_inference=False):
        x = super().get_input(batch, k, is_inference) #抓到target image 做channel轉變 以及變成tensor
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        #這裡concat
        # pose = batch['target_skeleton']
        # control = torch.cat([x,pose] , dim=1)
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c = self.get_learned_conditioning(xc.to(self.device))# text condition 怎拚
            else:
                c = xc
            if bs is not None:
                c = c[:bs]
    ...
    def shared_step(self, batch, **kwargs):
        x, c = self.get_input(batch, self.first_stage_key)# x, dict(c_crossattn=[c], c_concat=[control]) 得到condition以及x input
        loss = self(x, c)
        return loss
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.global_step % 7403/2 == 0: ##1000/4 accmulate gradient
    ...
    def draw_loss_curve(self, losses_curve,step):
        ...
        plt.legend()    
        index = 3
        filename = "idea4_aff_fix" + str(index) + ".jpg"
