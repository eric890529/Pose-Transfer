...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if context is None: #有兩個attention 第一個都是做self attention
            context = default(context, x)
        elif context.shape[2] != 768:
            ##
            #b, c, *spatial = x.shape #b h*w c
            #x = x.reshape(b, c, -1)
            context = rearrange(context, 'b c h w -> b (h w) c').contiguous()
            #context = context.reshape(b, h_w, -1)# b c h w
            ##
        ##
        # layer = nn.Linear(context.shape[2], self.to_k.in_features, bias=False).to("cuda") #因為權重的size就是768(CLIP的大小)所以這邊硬讓他變768
        # context = layer(context)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None): ##改過backword的東西
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 ...
                 use_checkpoint=True, is_contorlNet =False):
        super().__init__()
