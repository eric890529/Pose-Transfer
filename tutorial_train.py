...
# resume_path = './checkpoint_for_idea4_all/new_exp_sd21_epoch=187_step=579000.ckpt'
#batch_size = 2
logger_freq = 3000
learning_rate = 1e-5
sd_locked = True
...
import os
directory = "checkpoint_for_idea4_all_firstLayer"
if not os.path.exists(directory):
    os.makedirs(directory)
acc_size = 2
checkpoint_callback = ModelCheckpoint(dirpath = directory,
                                      save_top_k = -1,
                                      every_n_train_steps=7000, save_last=True, #4000/1000
...
trainer = pl.Trainer(accelerator="gpu", devices=[gpu], precision=32, callbacks=[logger, checkpoint_callback], accumulate_grad_batches=acc_size)# , resume_from_checkpoint = './checkpoint/last.ckpt' , resume_from_checkpoint = './checkpoint_for_diffusion/last.ckpt'
